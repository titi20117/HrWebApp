    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<PersonalityTest>(entity =>
        {
            ...
            entity.HasOne(d => d.User).WithMany(p => p.PersonalityTests)
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK__Personali__user___02084FDA");
        });
        ...
        {
            entity.HasKey(e => new { e.VacancyId, e.SkillId });

            entity.ToTable("Vacancies_Skills");

            entity.Property(e => e.VacancyId).HasColumnName("vacancy_id");
            entity.Property(e => e.SkillId).HasColumnName("skill_id");

            entity.HasOne(d => d.Skill).WithMany(p => p.VacanciesSkills)
                .HasForeignKey(d => d.SkillId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK__Vacancies__skill__6754599E");

            entity.HasOne(d => d.SkillNavigation).WithMany(p => p.VacanciesSkills)
                .HasForeignKey(d => d.SkillId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK__Vacancies__vacan__66603565");
        });
